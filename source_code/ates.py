...
def add_ates_equations(m=None):
    ...
    def ates_c_fix(m, s, y):
        if (y - 5) in m.set_years:
            return m.v_ates_c_fix[s, y] == m.p_scenario_weighting[s] * (m.v_ates_c_fix[s, y-5] + (m.v_ates_k_inv[s, y] * m.p_ates_c_inv[s, y] + m.v_ates_hp_Q_inv[s, y] * m.p_hp_c_inv[s, y]) * 0.02 + (m.v_ates_k_thermal_max[y] - m.v_ates_k_thermal_max[y-5]) * m.p_c_mean_elec[s, y] * m.p_ates_elec[s, y])
            ...
            return m.v_ates_c_fix[s, y] == m.p_scenario_weighting[s] * ((m.v_ates_k_inv[s, y] * m.p_ates_c_inv[s, y] + m.v_ates_hp_Q_inv[s, y] * m.p_hp_c_inv[s, y]) * 0.02 + m.v_ates_k_thermal_max[y] * m.p_c_mean_elec[s, y] * m.p_ates_elec[s, y])
    ...
    def ates_c_var(m, s, y, t):
        return m.v_ates_c_var[s, y, t] == m.p_scenario_weighting[s] * (m.v_ates_q_elec_consumption[s, y, t] * m.p_c_elec[s, y, t] + (m.v_ates_q_thermal_in[s, y, t] + m.v_ates_q_thermal_out[s, y, t]) * m.p_ates_c_charge_discharge[s, y])
