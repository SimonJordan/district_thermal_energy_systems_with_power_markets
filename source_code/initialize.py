...
def initialize_parameters(years=[]):
    ...
    path_to_file_hp_cop = os.path.join(path_to_input_folder, 'temperature.xlsx')
    path_to_file_solar_radiation = os.path.join(path_to_input_folder, 'solar_radiation.xlsx')
    ...
    path_to_file_inv_capacity = os.path.join(path_to_input_folder, 'inv_capacity.xlsx')
    path_to_file_eb = os.path.join(path_to_input_folder, 'eb.xlsx')
    path_to_file_gb = os.path.join(path_to_input_folder, 'gb.xlsx')
    ...
    path_to_file_btes = os.path.join(path_to_input_folder, 'btes.xlsx')
    ...
    for year in years:
        ...
        df_gb = pd.read_excel(path_to_file_gb, sheet_name=str(year))
        ...
        df_btes = pd.read_excel(path_to_file_btes, sheet_name=str(year))
        ...
        electricity_price[year] = df_electricity_price['electricity_price_NECPEssentials_mean'].tolist()
        ...
        p_eb_inv = df_inv_capacity['eb'].tolist()[0]
        data_eb[year] = {'p_eb_eta': p_eb_eta, 'p_eb_c_inv': p_eb_c_inv, 'p_eb_inv': p_eb_inv}
        p_gb_eta = df_gb['p_gb_eta'].tolist()[0]
        p_gb_h_gas = df_gb['p_gb_h_gas'].tolist()[0]
        p_gb_co2_share = df_gb['p_gb_co2_share'].tolist()[0]
        p_gb_c_inv = df_gb['p_gb_c_inv'].tolist()[0]
        p_gb_inv = df_inv_capacity['gb'].tolist()[0]
        data_gb[year] = {'p_gb_eta': p_gb_eta, 'p_gb_h_gas': p_gb_h_gas, 'p_gb_co2_share': p_gb_co2_share, 'p_gb_c_inv': p_gb_c_inv, 'p_gb_inv': p_gb_inv}
        p_hp_c_inv = df_hp['p_hp_c_inv'].tolist()[0]
        p_hp_cop = df_hp_cop['p_hp_cop'].tolist()
        p_hp_inv = df_inv_capacity['hp'].tolist()[0]
        data_hp[year] = {'p_hp_c_inv': p_hp_c_inv, 'p_hp_cop': p_hp_cop, 'p_hp_inv': p_hp_inv}
        ...
        p_st_inv = df_inv_capacity['st'].tolist()[0]
        data_st[year] = {'p_st_eta': p_st_eta, 'p_st_c_inv': p_st_c_inv, 'p_st_cop': p_st_cop, 'p_st_solar_radiation': p_st_solar_radiation, 'p_st_inv': p_st_inv}
        ...
        p_wi_inv = df_inv_capacity['wi'].tolist()[0]
        data_wi[year] = {'p_wi_eta': p_wi_eta, 'p_wi_q_waste': p_wi_q_waste, 'p_wi_c_waste': p_wi_c_waste, 'p_wi_h_waste': p_wi_h_waste, 'p_wi_heat': p_wi_heat, 'p_wi_elec': p_wi_elec, 'p_wi_co2_share': p_wi_co2_share, 'p_wi_c_inv': p_wi_c_inv, 'p_wi_inv': p_wi_inv}
        ...
        p_ieh_inv = df_inv_capacity['ieh'].tolist()[0]
        data_ieh[year] = {'p_ieh_c_inv': p_ieh_c_inv, 'p_ieh_c_in': p_ieh_c_in, 'p_ieh_elec': p_ieh_elec, 'p_ieh_in': p_ieh_in, 'p_ieh_inv': p_ieh_inv}
        ...
        p_chp_inv = df_inv_capacity['chp'].tolist()[0]
        data_chp[year] = {'p_chp_eta': p_chp_eta, 'p_chp_h_gas': p_chp_h_gas, 'p_chp_heat': p_chp_heat, 'p_chp_elec': p_chp_elec, 'p_chp_co2_share': p_chp_co2_share, 'p_chp_c_inv': p_chp_c_inv, 'p_chp_inv': p_chp_inv}
        p_ac_eer = df_ac_eer['p_ac_eer'].tolist()
        p_ac_c_inv = df_ac['p_ac_c_inv'].tolist()[0]
        p_ac_inv = df_inv_capacity['ac'].tolist()[0]
        data_ac[year] = {'p_ac_eer': p_ac_eer, 'p_ac_c_inv': p_ac_c_inv, 'p_ac_inv': p_ac_inv}
        ...
        p_ab_ct_inv = df_inv_capacity['ab_ct'].tolist()[0]
        p_ab_hp_inv = df_inv_capacity['ab_hp'].tolist()[0]
        data_ab[year] = {'p_ab_eer': p_ab_eer, 'p_ab_hp_cop': p_ab_hp_cop, 'p_ab_c_inv': p_ab_c_inv, 'p_ct_ab_c_inv': p_ct_ab_c_inv, 'p_ab_ct_inv': p_ab_ct_inv, 'p_ab_hp_inv': p_ab_hp_inv}
        ...
        p_cp_ct_inv = df_inv_capacity['cp_ct'].tolist()[0]
        p_cp_hp_inv = df_inv_capacity['cp_hp'].tolist()[0]
        data_cp[year] = {'p_cp_ct_seer': p_cp_ct_seer, 'p_cp_hp_seer': p_cp_hp_seer, 'p_cp_hp_cop': p_cp_hp_cop, 'p_cp_c_inv': p_cp_c_inv, 'p_ct_cp_c_inv': p_ct_cp_c_inv, 'p_cp_ct_inv': p_cp_ct_inv, 'p_cp_hp_inv': p_cp_hp_inv}
        ...
        p_ttes_inv = df_inv_capacity['ttes'].tolist()[0]
        p_ttes_hp_inv = df_inv_capacity['ttes_hp'].tolist()[0]
        data_ttes[year] = {'p_ttes_losses': p_ttes_losses, 'p_ttes_eta': p_ttes_eta, 'p_ttes_init': p_ttes_init, 'p_ttes_end': p_ttes_end, 'p_ttes_c_inv': p_ttes_c_inv, 'p_ttes_elec': p_ttes_elec, 'p_ttes_cop': p_ttes_cop, 'p_ttes_c_charge_discharge': p_ttes_c_charge_discharge, 'p_ttes_inv': p_ttes_inv, 'p_ttes_hp_inv': p_ttes_hp_inv}
        p_btes_losses = df_btes['p_btes_losses'].tolist()[0]
        p_btes_eta = df_btes['p_btes_eta'].tolist()[0]
        p_btes_init = df_btes['p_btes_init'].tolist()[0]
        p_btes_end = df_btes['p_btes_end'].tolist()[0]
        p_btes_c_inv = df_btes['p_btes_c_inv'].tolist()[0]
        p_btes_elec = df_btes['p_btes_elec'].tolist()[0]
        p_btes_cop = df_btes['p_btes_cop'].tolist()[0]
        p_btes_c_charge_discharge = df_btes['p_btes_c_charge_discharge'].tolist()[0]
        p_btes_inv = df_inv_capacity['btes'].tolist()[0]
        p_btes_hp_inv = df_inv_capacity['btes_hp'].tolist()[0]
        data_btes[year] = {'p_btes_losses': p_btes_losses, 'p_btes_eta': p_btes_eta, 'p_btes_init': p_btes_init, 'p_btes_end': p_btes_end, 'p_btes_c_inv': p_btes_c_inv, 'p_btes_elec': p_btes_elec, 'p_btes_cop': p_btes_cop, 'p_btes_c_charge_discharge': p_btes_c_charge_discharge, 'p_btes_inv': p_btes_inv, 'p_btes_hp_inv': p_btes_hp_inv}
        ...
        p_ites_inv = df_inv_capacity['ites'].tolist()[0]
        p_ites_ac_inv = df_inv_capacity['ites_ac'].tolist()[0]
        data_ites[year] = {'p_ites_losses': p_ites_losses, 'p_ites_eta': p_ites_eta, 'p_ites_init': p_ites_init, 'p_ites_end': p_ites_end, 'p_ites_c_inv': p_ites_c_inv, 'p_ites_elec': p_ites_elec, 'p_ites_seer': p_ites_seer, 'p_ites_c_charge_discharge': p_ites_c_charge_discharge, 'p_ites_inv': p_ites_inv, 'p_ites_ac_inv': p_ites_ac_inv}

    return heating_demand, cooling_demand, electricity_price, electricity_mean_price, gas_price, co2_price, data_eb, data_gb, data_hp, data_st, data_wi, data_ieh, data_chp, data_ac, data_ab, data_cp, data_ttes, data_btes, data_ites