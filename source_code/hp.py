...
def add_hp_equations(m=None):
    ...
    def hp_elec_heat(m, y, t, s): 
        return m.v_hp_q_heat_in[y, t, s] == m.v_hp_q_elec_in[y, t, s] * m.p_hp_cop[y, s, t]
     
    def hp_Q_inv(m, y, s):
        if (y - 5) in m.set_years:
            return m.v_hp_Q_diff[y, s] == (m.v_hp_Q_heat_max[y, s] - m.v_hp_Q_heat_max[y-5, s])
        else:
            return m.v_hp_Q_diff[y, s] == m.v_hp_Q_heat_max[y, s]
    
    def hp_c_fix(m, y, s):
        return m.v_hp_c_fix[y, s] == m.v_hp_Q_diff[y, s] * m.p_hp_c_inv[y, s] + m.p_hp_c_fix[y, s] * m.v_hp_Q_heat_max[y, s]        
...
def add_hp_variables(m=None):
    ...
    m.v_hp_Q_diff = py.Var(m.set_years, m.set_scenarios,
                           domain = py.NonNegativeReals,
...
def add_hp_parameters(m=None):
    ...
    m.p_hp_cop = py.Parameter(m.set_years, m.set_hours, m.set_scenarios,
                              initialize = init_hp_cop,
                              within = py.NonNegativeReals,
                              doc = 'coefficient of performance of the large-scale heat pump')

    m.p_hp_c_inv =py.Parameter(m.set_years, m.set_scenarios,
