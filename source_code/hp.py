...
def add_hp_equations(m=None):
    ...
    def hp_Q_inv(m, s, y):
        if (y - 5) in m.set_years:
            return m.v_hp_Q_inv[s, y] == (m.v_hp_Q_heat_max[y] - m.v_hp_Q_heat_max[y-5])
        else:
            return m.v_hp_Q_inv[s, y] == m.v_hp_Q_heat_max[y]
    
    def hp_c_inv(m, s, y):
        return m.v_hp_c_inv[s, y] == m.p_scenario_weighting[s] * (m.v_hp_Q_inv[s, y] * m.p_hp_c_inv[s, y])
  
    def hp_c_fix(m, s, y):
        if (y - 5) in m.set_years:
            return m.v_hp_c_fix[s, y] == m.p_scenario_weighting[s] * (m.v_hp_c_fix[s, y-5] + m.v_hp_Q_inv[s, y] * m.p_hp_c_inv[s, y] * 0.02)
        else:
            return m.v_hp_c_fix[s, y] == m.p_scenario_weighting[s] * (m.v_hp_Q_inv[s, y] * m.p_hp_c_inv[s, y] * 0.02)

    def hp_c_var(m, s, y, t): # OPAM = operational and maintanance, förderung?
        return m.v_hp_c_var[s, y, t] == m.p_scenario_weighting[s] * (m.v_hp_q_elec_consumption[s, y, t] * (m.p_c_elec[s, y, t] + m.p_elec_co2_share[s, y, t] * m.p_c_co2[s, y]))
...
def add_hp_variables(m=None):
    ...
    m.v_hp_Q_inv = py.Var(m.set_scenarios, m.set_years,
                          domain = py.NonNegativeReals,
