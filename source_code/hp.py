...
def add_hp_equations(m=None):
    """This section defines the equations of the heat pump"""
    ...
    def hp_Q_inv(m, s, y):
        if (y - 5) in m.set_years:
            return m.v_hp_Q_inv[s, y] == m.v_hp_Q_heat_max[s, y] - m.v_hp_Q_heat_max[s, y-5]
        else:
            return m.v_hp_Q_inv[s, y] == m.v_hp_Q_heat_max[s, y]
    
    def hp_c_inv(m, s, y):
        return m.v_hp_c_inv[s, y] == m.v_hp_Q_inv[s, y] * m.p_hp_c_inv[s, y]
    ...
    def hp_c_var(m, s, y, h):
        return m.v_hp_c_var[s, y, h] == m.p_year_expansion_range[s, y] * (m.v_hp_q_elec_consumption[s, y, h] * (m.p_c_elec[s, y, h] + m.p_elec_co2_share[s, y, h] * m.p_c_co2[s, y]))
...
def add_hp_variables(m=None):
    """This section defines the variables of the heat pump"""
    ...
    m.v_hp_Q_heat_max = py.Var(m.set_scenarios, m.set_years,
                               domain = py.NonNegativeReals,
...
def add_hp_parameters(m=None):
    """This section defines the parameters of the heat pump"""
