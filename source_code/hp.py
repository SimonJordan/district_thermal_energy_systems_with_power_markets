...
def add_hp_equations(m=None):
    ...
    def hp_c_fix(m, s, y):
        if (y - 5) in m.set_years:
            return m.v_hp_c_fix[s, y] == m.v_hp_c_fix[s, y-5] + m.p_scenario_weighting[s] * m.p_year_expansion_range[s, y] * (m.v_hp_Q_inv[s, y] * m.p_hp_c_inv[s, y] * 0.02)
        else:
            return m.v_hp_c_fix[s, y] == m.p_scenario_weighting[s] * m.p_year_expansion_range[s, y] * (m.v_hp_Q_inv[s, y] * m.p_hp_c_inv[s, y] * 0.02)
