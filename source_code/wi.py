...
def add_wi_equations(m=None):
    ...
    def wi_c_inv(m, s, y):
        return m.v_wi_c_inv[s, y] == m.v_wi_Q_inv[s, y] * m.p_wi_c_inv[s, y]
   
    def wi_c_fix(m, s, y):
        if (y - 5) in m.set_years:
            return m.v_wi_c_fix[s, y] == m.v_wi_c_fix[s, y-5] + m.v_wi_Q_inv[s, y] * m.p_wi_c_inv[s, y] * 0.02
        else:
            return m.v_wi_c_fix[s, y] == m.v_wi_Q_inv[s, y] * m.p_wi_c_inv[s, y] * 0.02
    
    def wi_c_var(m, s, y, t): # OPAM = operational and maintanance, förderung?
        return m.v_wi_c_var[s, y, t] == m.p_wi_q_waste[s, y] * m.p_wi_c_waste[s, y] * m.v_wi_p_scale[s, y, t] + m.p_wi_q_waste[s, y] * m.p_wi_co2_share[s, y] * m.p_c_co2[s, y] * m.v_wi_p_scale[s, y, t] - m.v_wi_q_elec_in[s, y, t] * m.p_c_elec[s, y, t]
...
def add_wi_variables(m=None):
    ...
    m.v_wi_Q_mix_max = py.Var(m.set_scenarios, m.set_years,
                              domain = py.NonNegativeReals,
