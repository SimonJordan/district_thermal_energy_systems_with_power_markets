...
def add_ites_equations(m=None):
    ...
    def ites_c_fix(m, s, y):
        if (y - 5) in m.set_years:
            return m.v_ites_c_fix[s, y] == m.v_ites_c_fix[s, y-5] + m.p_year_expansion_range[s, y] * (m.v_ites_c_inv[s, y] * 0.02 + m.v_ites_k_inv[s, y] * m.p_c_mean_elec[s, y] * m.p_ites_elec[s, y])
        else:
            return m.v_ites_c_fix[s, y] == m.p_year_expansion_range[s, y] * (m.v_ites_c_inv[s, y] * 0.02 + m.v_ites_k_inv[s, y] * m.p_c_mean_elec[s, y] * m.p_ites_elec[s, y])
