...
def add_ites_equations(m=None):
    ...
    def ites_soc_max_bound(m, s, y, h):
        return m.v_ites_k_cool[s, y, h] <= m.v_ites_k_cool_max[y]

    def ites_soc(m, s, y, h):
        if h == 0:
            if y == 2025:
    ...
    def ites_elec_cool(m, s, y, h):
        return m.v_ites_q_cool_out[s, y, h] == m.v_ites_q_elec_consumption[s, y, h] * m.p_ites_seer[s, y]
    ...
    def ites_c_var(m, s, y, h):
        return m.v_ites_c_var[s, y, h] == m.p_scenario_weighting[s] * m.p_year_expansion_range[s, y] * (m.v_ites_q_elec_consumption[s, y, h] * (m.p_c_elec[s, y, h] + m.p_elec_co2_share[s, y, h] * m.p_c_co2[s, y]) + (m.v_ites_q_cool_in[s, y, h] + m.v_ites_q_cool_out[s, y, h]) * m.p_ites_c_charge_discharge[s, y])
