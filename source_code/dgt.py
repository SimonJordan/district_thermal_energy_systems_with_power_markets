...
def add_dgt_equations(m=None):
    ...
    def dgt_Q_inv(m, s, y):
        if (y - 5) in m.set_years:
            return m.v_dgt_Q_inv[s, y] == (m.v_dgt_Q_heat_max[y] - m.v_dgt_Q_heat_max[y-5])
        else:
            return m.v_dgt_Q_inv[s, y] == m.v_dgt_Q_heat_max[y]
    
    def dgt_c_inv(m, s, y):
        return m.v_dgt_c_inv[s, y] == m.p_scenario_weighting[s] * (m.v_dgt_Q_inv[s, y] * m.p_dgt_c_inv[s, y])
  
    def dgt_c_fix(m, s, y):
        if (y - 5) in m.set_years:
            return m.v_dgt_c_fix[s, y] == m.v_dgt_c_fix[s, y-5] + m.p_year_expansion_range[s, y] * (m.v_dgt_Q_inv[s, y] * m.p_dgt_c_inv[s, y] * 0.02)
            return m.v_dgt_c_fix[s, y] == m.p_scenario_weighting[s] * (m.v_dgt_c_fix[s, y-5] + m.v_dgt_Q_inv[s, y] * m.p_dgt_c_inv[s, y] * 0.02)
        else:
            return m.v_dgt_c_fix[s, y] == m.p_year_expansion_range[s, y] * (m.v_dgt_Q_inv[s, y] * m.p_dgt_c_inv[s, y] * 0.02)
            return m.v_dgt_c_fix[s, y] == m.p_scenario_weighting[s] * (m.v_dgt_Q_inv[s, y] * m.p_dgt_c_inv[s, y] * 0.02)

    def dgt_c_var(m, s, y, t):
        return m.v_dgt_c_var[s, y, t] == m.p_year_expansion_range[s, y] * (m.v_dgt_q_heat_in[s, y, t] * 0.1 * (m.p_c_elec[s, y, t] + m.p_elec_co2_share[s, y, t] * m.p_c_co2[s, y]))

    def dgt_c_var(m, s, y, t): # OPAM = operational and maintanance, förderung?
        return m.v_dgt_c_var[s, y, t] == m.p_scenario_weighting[s] * (m.v_dgt_q_heat_in[s, y, t] * 0.1 * (m.p_c_elec[s, y, t] + m.p_elec_co2_share[s, y, t] * m.p_c_co2[s, y]))
