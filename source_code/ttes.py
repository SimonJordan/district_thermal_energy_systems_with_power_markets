...
def add_ttes_equations(m=None):
    ...
    def ttes_soc(m, s, y, h):
        if h == 0:
            ...
            if y == 2025:
                return m.v_ttes_k_heat[s, y, h] == m.v_ttes_k_heat_max[s, y] * m.p_ttes_losses[s, y] * m.p_ttes_init[s, y] + m.v_ttes_q_heat_out[s, y, h] * m.p_ttes_eta[s, y] - m.v_ttes_q_heat_in[s, y, h] / m.p_ttes_eta[s, y]
            else:
                return m.v_ttes_k_heat[s, y, h] == m.v_ttes_k_heat[s, y-5, 8759] * m.p_ttes_losses[s, y] + m.v_ttes_q_heat_out[s, y, h] * m.p_ttes_eta[s, y] - m.v_ttes_q_heat_in[s, y, h] / m.p_ttes_eta[s, y]
        elif h == 8759 and y == 2050:
            return m.v_ttes_k_heat[s, y, h] == m.v_ttes_k_heat_max[s, y] * m.p_ttes_end[s, y]
        else:
    ...
    def ttes_hp_Q_inv(m, y):
        if (y - 5) in m.set_years:
            ...
            return m.v_ttes_hp_Q_inv[y] == m.v_ttes_hp_Q_max[y]
   
    def ttes_c_inv(m, s, y):
        return m.v_ttes_c_inv[s, y] == m.p_scenario_weighting[s] * (m.v_ttes_k_inv[y] * m.p_ttes_c_inv[s, y] + m.v_ttes_hp_Q_inv[y] * m.p_hp_c_inv[s, y])
    
    def ttes_c_fix(m, s, y):
        if (y - 5) in m.set_years:
            return m.v_ttes_c_fix[s, y] == m.v_ttes_c_fix[s, y-5] + m.p_scenario_weighting[s] * m.p_year_expansion_range[s, y] * (m.v_ttes_c_inv[s, y] * 0.02 + m.v_ttes_k_inv[y] * (m.p_c_mean_elec[s, y] + m.p_mean_elec_co2_share[s, y] * m.p_c_co2[s, y]) * m.p_ttes_elec[s, y])
        else:
            return m.v_ttes_c_fix[s, y] == m.p_scenario_weighting[s] * m.p_year_expansion_range[s, y] * (m.v_ttes_c_inv[s, y] * 0.02 + m.v_ttes_k_inv[y] * (m.p_c_mean_elec[s, y] + m.p_mean_elec_co2_share[s, y] * m.p_c_co2[s, y]) * m.p_ttes_elec[s, y])
    ...
    # m.con_ttes_limit_1 = py.Constraint(m.set_years,
    #                                    rule = ttes_limit_1)
    
    # m.con_ttes_limit_2 = py.Constraint(m.set_years,
    #                                    rule = ttes_limit_2)
...
def add_ttes_variables(m=None):
    ...
    m.v_ttes_k_inv = py.Var(m.set_years,
                            domain = py.NonNegativeReals,
                            doc = 'new installed capacity of TTES per scenario and year')
   
    m.v_ttes_hp_Q_inv = py.Var(m.set_years,
                               domain = py.NonNegativeReals,
