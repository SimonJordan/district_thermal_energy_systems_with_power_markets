...
# scenarios_weighting = {'1_reference': 0.15, '2_high_electricity_prices': 0.05, '3_low_electricity_prices': 0.03, '4_flexible_energy_market': 0.1, '5_energy_congestion': 0.06, '6_green_friendly': 0.06, '7_low_gas_demand': 0.06, '8_natural_gas_friendly': 0.03, '9_cold_winters': 0.1, '10_hot_summers': 0.1, '11_warm_summers': 0.03, '12_moderate_climate': 0.03, '13_stagnating_co2_pricing': 0.04, '14_ambitious_co2_pricing': 0.1, '15_expiring_support_res': 0.06}
...
def objective_function(m):
    return sum(scenarios_weighting[s] * m.v_eb_c_inv[s, y] for s in m.set_scenarios for y in m.set_years) + sum(scenarios_weighting[s] * m.v_eb_c_fix[s, y] for s in m.set_scenarios for y in m.set_years) + sum(scenarios_weighting[s] * m.v_eb_c_var[s, y, h] for s in m.set_scenarios for y in m.set_years for h in m.set_hours) + \
           sum(scenarios_weighting[s] * m.v_gb_c_inv[s, y] for s in m.set_scenarios for y in m.set_years) + sum(scenarios_weighting[s] * m.v_gb_c_fix[s, y] for s in m.set_scenarios for y in m.set_years) + sum(scenarios_weighting[s] * m.v_gb_c_var[s, y, h] for s in m.set_scenarios for y in m.set_years for h in m.set_hours) + \
           ...
           sum(scenarios_weighting[s] * m.v_btes_c_inv[s, y] for s in m.set_scenarios for y in m.set_years) + sum(scenarios_weighting[s] * m.v_btes_c_fix[s, y] for s in m.set_scenarios for y in m.set_years) + sum(scenarios_weighting[s] * m.v_btes_c_var[s, y, h] for s in m.set_scenarios for y in m.set_years for h in m.set_hours) + \
