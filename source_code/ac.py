...
def add_ac_equations(m=None):
    ...
    def ac_c_inv(m, s, y):
        return m.v_ac_c_inv[s, y] == m.p_scenario_weighting[s] * m.v_ac_Q_inv[y] * m.p_ac_c_inv[s, y]
  
    def ac_c_fix(m, s, y):
        if (y - 5) in m.set_years:
            return m.v_ac_c_fix[s, y] == m.v_ac_c_fix[s, y-5] + m.p_scenario_weighting[s] * m.p_year_expansion_range[s, y] * m.v_ac_c_inv[s, y] * 0.02
        else:
            return m.v_ac_c_fix[s, y] == m.p_scenario_weighting[s] * m.p_year_expansion_range[s, y] * m.v_ac_c_inv[s, y] * 0.02

    def ac_c_var(m, s, y, t):
        return m.v_ac_c_var[s, y, t] == m.p_scenario_weighting[s] * m.p_year_expansion_range[s, y] * (m.v_ac_q_elec_consumption[s, y, t] * (m.p_c_elec[s, y, t] + m.p_elec_co2_share[s, y, t] * m.p_c_co2[s, y]))
    ...
    m.con_ac_Q_inv = py.Constraint(m.set_years,
                                   rule = ac_Q_inv)
