...
def add_gt_equations(m=None):
    """This section defines the equations of the geothermal"""
    def gt_feed_in_max_bound(m, s, y, h):
        return m.v_gt_q_heat_in[s, y, h] <= m.v_gt_Q_heat_max[s, y]
    
    # def gt_limit(m, s, y):
    #     return m.v_gt_Q_heat_max[s, y] <= 0
    ...
    def gt_c_fix(m, s, y):
        if (y - 5) in m.set_years:
            return m.v_gt_c_fix[s, y] == m.v_gt_c_fix[s, y-5] + m.p_year_expansion_range[s, y] * m.v_gt_c_inv[s, y] * 0.02
        else:
            return m.v_gt_c_fix[s, y] == m.p_year_expansion_range[s, y] * m.v_gt_c_inv[s, y] * 0.02

    def gt_c_var(m, s, y, h):
        return m.v_gt_c_var[s, y, h] == m.p_year_expansion_range[s, y] * (m.v_gt_q_elec_consumption[s, y, h] * (m.p_c_elec[s, y, h] + m.p_elec_co2_share[s, y, h] * m.p_c_co2[s, y]))
...
def add_gt_variables(m=None):
    """This section defines the variables of the geothermal"""
...
def add_gt_parameters(m=None):
    """This section defines the parameters of the geothermal"""
