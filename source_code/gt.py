...
def add_gt_equations(m=None):
    ...
    def gt_c_inv(m, s, y):
        return m.v_gt_c_inv[s, y] == m.p_scenario_weighting[s] * (m.v_gt_Q_inv[s, y] * (m.p_gt_c_inv[s, y] + m.p_hp_c_inv[s, y]))
  
    def gt_c_fix(m, s, y):
        if (y - 5) in m.set_years:
            return m.v_gt_c_fix[s, y] == m.v_gt_c_fix[s, y-5] + m.p_scenario_weighting[s] * m.p_year_expansion_range[s, y] * (m.v_gt_Q_inv[s, y] * (m.p_gt_c_inv[s, y] + m.p_hp_c_inv[s, y]) * 0.02)
        else:
            return m.v_gt_c_fix[s, y] == m.p_scenario_weighting[s] * m.p_year_expansion_range[s, y] * (m.v_gt_Q_inv[s, y] * (m.p_gt_c_inv[s, y] + m.p_hp_c_inv[s, y]) * 0.02)
