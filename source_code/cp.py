...
def add_cp_equations(m=None):
    ...
    def cp_ct_limit(m, s, y):
        return m.v_cp_ct_Q_inv[s, y] == m.p_cp_ct_inv[s, y]
    ...
    def cp_ct_c_var(m, s, y, h):
        return m.v_cp_ct_c_var[s, y, h] == m.p_year_expansion_range[s, y] * (m.v_cp_ct_q_elec_consumption[s, y, h] * m.p_c_elec[s, y, h])

    def cp_hp_feed_in_max_bound(m, s, y, h):
        return m.v_cp_hp_q_cool_in[s, y, h] <= m.v_cp_hp_Q_cool_max[s, y]
    
    def cp_hp_limit(m, s, y):
        return m.v_cp_hp_Q_inv[s, y] == m.p_cp_hp_inv[s, y]
    ...
    def cp_hp_c_inv(m, s, y):
        return m.v_cp_hp_c_inv[s, y] == m.v_cp_hp_Q_inv[s, y] * (m.p_cp_c_inv[s, y] + m.p_hp_c_inv[s, y] * (1 + 1 / m.p_cp_hp_seer[s, y]))
    ...
    def cp_hp_c_var(m, s, y, h):
        return m.v_cp_hp_c_var[s, y, h] == m.p_year_expansion_range[s, y] * (m.v_cp_hp_q_elec_consumption[s, y, h] * m.p_c_elec[s, y, h])
    ...
    m.con_cp_hp_Q_inv = py.Constraint(m.set_scenarios, m.set_years,
                                      rule = cp_hp_Q_inv)
...
def add_cp_variables(m=None):
    ...
    m.v_cp_hp_Q_cool_max = py.Var(m.set_scenarios, m.set_years,
                                  domain = py.NonNegativeReals,
                                  doc = 'max cool feed in from large-scale compressor with heat pump for district cooling')
    
    m.v_cp_hp_Q_inv = py.Var(m.set_scenarios, m.set_years,
                             domain = py.NonNegativeReals,
