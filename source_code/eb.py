...
def add_eb_equations(m=None):
    ...
    def eb_Q_inv(m, y, s):
        if (y - 5) in m.set_years:
            return m.v_eb_Q_diff[y, s] == m.v_eb_Q_heat_max[y, s] - m.v_eb_Q_heat_max[y-5, s]
        else:
            return m.v_eb_Q_diff[y, s] == m.v_eb_Q_heat_max[y, s]
    
    def eb_c_fix(m, y, s):
        return m.v_eb_c_fix[y, s] == m.v_eb_Q_diff[y, s] * m.p_eb_c_inv[y, s] + m.p_eb_c_fix[y, s] * m.v_eb_Q_heat_max[y, s]
...
def add_eb_variables(m=None):
    ...
    m.v_eb_Q_diff = py.Var(m.set_years, m.set_scenarios,
                           domain = py.NonNegativeReals,
