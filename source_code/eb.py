...
def add_eb_equations(m=None):
    ...
    def eb_elec_heat(m, y, t, s):
        m.v_eb_q_heat_in[y, t, s] == m.v_eb_q_elec_in[y, t, s] * m.p_eb_eta
        
    def eb_c_fix(m, y, t, s): #y=0 berücksichtigen fehlt, größer gleich im vergleich zum vorjahr
        m.v_eb_c_fix(y, s) == (m.v_eb_Q_heat_max[y, s] - m.v_eb_Q_heat_max[y-5, s]) * m.p_eb_c_inv(y, s)
        
    def eb_c_var(m, y, t, s): # OPAM = operational and maintanance, förderung?
        m.v_eb_c_var(y, s) == m.v_eb_q_elec_in[y, t, s] * m.p_c_elec[y, t, s]
...
def add_eb_parameters(m=None):
    m.p_eb_eta = py.Parameter( #Einlesen von Inputs fehlt!
        within = py.NonNegativeReals,
        doc = 'electrical efficiency of the eb'
    )
    m.p_eb_c_inv =py.Parameter( #Einlesen von Inputs fehlt!
        m.set_years * m.set_scenarios,
        within = py.NonNegativeReals,
